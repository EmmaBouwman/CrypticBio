...
def process_and_save_attention(attn_map, raw_img, output_path):
    ...
    attn_map_resized = (attn_map_resized - attn_map_min) / (
        attn_map_max - attn_map_min + 1e-8
    )
...
def main():
    parser = argparse.ArgumentParser(description="Generate Raw Attention visualization")
    parser.add_argument(
        "--model_path", type=str, default="best_animal_sat_base_resized.pth"
    )
    parser.add_argument(
        "--model_type",
        type=int,
        ...
        help="""Model to be used:
                            1 = Only Animal image, 
                            2 = Only Satelite image, 
                            3 = Cross attention (both images)""",
    )
    ...
    if model_type == ModelType.Both:
        ...
        raise ValueError(f"Model type {model_type} is not configured for this mapping.")
    ...
    all_ids = [
        int(os.path.splitext(f)[0]) for f in os.listdir(cb_folder) if f.endswith(".png")
    ]
    ...
    dataset = AnimalSateliteDataset(
        valid_ids, name_to_id, db_path, transform_size=224, model_type=model_type
    )
    ...
    if model_type == ModelType.Both:
        ...
        # Target block -3: Deep enough for semantics, avoids 'attention sink' bias
        attn_module = model.animal_backbone.blocks[-3].attn

        # Disable fused_attn to ensure the hook can access dropout input
        ...
        with torch.no_grad():
            logits, cross_attn_weights = model(
                animal_tensor, sat_tensor, attn_bool=True
            )
        ...
        animal_out_path = os.path.join(
            args.output_dir, f"attention_animal_{first_test_idx}.jpg"
        )
        ...
        sat_out_path = os.path.join(
            args.output_dir, f"attention_sat_{first_test_idx}.jpg"
        )
        ...
        image_out_path = os.path.join(
            args.output_dir, f"attention_{model_type.name}_{first_test_idx}.jpg"
        )
